...
law_effective_status = db[MongoDBCollectionConfig.LAW_EFFECTIVE_STATUS_COLLECTION_NAME]
...
class DocumentTextSearchAPI(Resource):
    ...
    def _validate_filters(self, filters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Sorting flags (optional). When set, the response is built and sorted
        # in-memory (buffered dict) instead of streamed.
        if filters.get('sortByDocType'):
            validated_filters['sortByDocType'] = True
        if filters.get('sortByIssuedDate'):
            validated_filters['sortByIssuedDate'] = True
    ...
    @staticmethod
    def _build_sort(validated_filters: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Build the ES sort array for the requested flags.

        - none           -> [] (default ES order)
        - sortByDocType  -> script sort by doc type NAME (type_id ranked via the
                            alphabetically-sorted MongoDB type list); unknown/missing
                            type_id sorts last
        - sortByIssuedDate -> issue date descending
        - both           -> doc type first (priority), issue date as tiebreaker

        Doc type sorting is pushed into ES via a {type_id: rank} param map, so the
        whole result set is ordered server-side (no per-type walking).
        """
        sort: List[Dict[str, Any]] = []
        if validated_filters.get('sortByDocType'):
            ordered_type_ids = [tid for tid, _ in sorted(get_doc_type_map().items(), key=lambda kv: kv[1])]
            rank_map = {tid: i for i, tid in enumerate(ordered_type_ids)}
            sort.append({
                "_script": {
                    "type": "number",
                    "script": {
                        "source": (
                            "if (doc['type_id.keyword'].size() == 0) { return params.miss; } "
                            "String t = doc['type_id.keyword'].value; "
                            "return params.rank.containsKey(t) ? params.rank[t] : params.miss;"
                        ),
                        "params": {"rank": rank_map, "miss": len(rank_map)},
                    },
                    "order": "asc",
                }
            })
        if validated_filters.get('sortByIssuedDate'):
            sort.append({"doc_issue_date.keyword": {"order": "desc"}})
        return sort

    @staticmethod
    def _sorted_hits_gen(query: Dict[str, Any], sort: List[Dict[str, Any]], skip: int, quantity: int):
        """Yield ES hits (in [hit] batches, matching enrich_stream's expectation)
        for a sorted query. Uses from/size, so deep pages (>10k) hit the ES
        max_result_window — acceptable for normal paging."""
    ...
    def post(self, page: int, quantity: int):
        ...
        try:
            ...
            # Pick the hit source: a sorted from/size query when a sort flag is set,
            # otherwise the default scroll. Both feed the same enrich_stream output.
            skip = (page - 1) * quantity
            sort = self._build_sort(validated_filters)
            logger.debug("search_elasticsearch_started", action="post", search_query=query, page=page, quantity=quantity, skip=skip, sort=sort)
            if sort:
                search_gen = self._sorted_hits_gen(query, sort, skip, quantity)
            else:
            ...
            duration = (datetime.now() - start_time).total_seconds()
            logger.info("search_documents_success", action="post", **{"event.duration": duration, "event.status": "success"}, sorted=bool(sort))
            # Stream response
            return Response(
...
class DocumentDeleteAPI(Resource):
    ...
    def _delete_from_mongo(self, idOrCode: str):
        try:
            ...
            # Xóa khỏi các cây chủ đề
            subjects = list(law_tree_manager.subject_tree_collection.find({"doc_id_includes": idOrCode}))
            for subject in subjects:
                law_tree_manager.remove_document_to_subject(subject_id=subject["subject_id"], doc_id=idOrCode)
            logger.debug("deleted_document_from_trees", action="_delete_from_mongo", doc_id=idOrCode, trees_updated_count=len(subjects))
...
class DocumentSemanticSearchAPI(Resource):
    ...
    def _semantic_search(self, validated_filters):
        ...
        logger.debug("semantic_search_response", action="_semantic_search", response=response.json())
    ...
    def post(self, page: int, quantity: int):
        ...
        try:
            ...
            if validated_filters['text']:
                ...
                score_map = {
                    str(result['document_id']): result['score']
                    for result in semantic_search_result['result']
                } 
                logger.debug("semantic_search_success", action="post", score_map=score_map)             
            ...
            if validated_filters.get('text'):
                search_gen = search(es_client, index=ElasticConfig.ELASTIC_INDEX, 
                                    query=query, 
                                    batch_size=1000,
                                    max_records=len(codes), 
                                    skip=0)                 
                # Normalize toàn bộ results về dict trước khi xử lý
                results = list(search_gen)
                if results and isinstance(results[0], list):
                    results = [doc for batch in results for doc in batch]
                results = [json.loads(doc) if isinstance(doc, str) else doc for doc in results]
                # Sort theo điểm từ Qdrant
                results.sort(key=lambda doc: score_map.get(str(doc['_source'].get('doc_id')), 0), reverse=True )
                results = results[skip : skip + quantity]
                search_gen = iter([results])
                if results:
                    logger.debug("sort_results_success", action="post", results=results[0]['_source'].get('doc_id'), score=results[0].get('_score'))
            else:
                # Nếu không có text (tìm kiếm thường), phân trang thẳng dưới ES để tối ưu
                search_gen = search(es_client, index=ElasticConfig.ELASTIC_INDEX, 
                                    query=query, 
                                    batch_size=1000,
                                    max_records=quantity,
            ...
            if doc_id and (doc_effective_status_id is not None or doc_expiry_date is not None):
                ...
                if doc_effective_status_id is not None:
                    ...
                    if doc_effective_status_id:
                        doc = law_effective_status.find_one({'effective_status_id': doc_effective_status_id})
            ...
            if article_id and (article_effective_status_id is not None or article_expiry_date is not None):
                ...
                if article_effective_status_id is not None:
                    ...
                    if article_effective_status_id:
                        doc = law_effective_status.find_one({'effective_status_id': article_effective_status_id})
...
class DocumentTextSearchExportAPI(Resource):
    """Export text-search results to .xlsx.

    Takes the same filter body as DocumentTextSearchAPI and always sorts: if no
    sort flag is sent it defaults to doc type A->Z then newest. Pulls the full
    result set via ES scroll (sorted, no 10k window limit). When the result set
    exceeds PART_SIZE it is split into multiple .xlsx files (Phần i/N) bundled in
    a .zip; STT increments continuously across parts. A single part is returned
    as a plain .xlsx.
    """

    PART_SIZE = 25000
    COLUMNS = ["STT", "Tên văn bản", "Số hiệu văn bản",
               "Ngày có hiệu lực", "Ngày hết hiệu lực", "Ngày ban hành"]

    @staticmethod
    def _format_date(value: Any) -> str:
        """Normalize a stored date string to dd/mm/YYYY. Returns "" for missing,
        unparseable, or placeholder dates (year < 1900, e.g. '1-01-01 00:00:00')."""
        if not value:
            return ""
        s = str(value).strip()
        for fmt in ("%Y-%m-%d %H:%M:%S", "%Y-%m-%dT%H:%M:%S", "%Y-%m-%d"):
            try:
                ...
                return "" if dt.year < 1900 else dt.strftime("%d/%m/%Y")
            except ValueError:
                continue
        return ""

    def _build_part(self, rows, part: int, num_parts: int, start_stt: int):
        """Build one styled .xlsx part from the row generator.

        Consumes up to PART_SIZE docs from `rows`, numbering STT from start_stt+1.
        Part info appears in the title (top) and a footer (bottom) only when the
        export spans more than one part. Returns (xlsx_bytes, last_stt).
        """
        ...
        for col_idx, width in enumerate([10, 50, 15, 15, 15, 15], start=1):
            ws.column_dimensions[get_column_letter(col_idx)].width = width

        # Row 1 — title (merged A1:B1), with part suffix when multi-part
        title = "DANH SÁCH VĂN BẢN QUY PHẠM PHÁP LUẬT"
        if num_parts > 1:
            title += f" (Phần {part}/{num_parts})"
        ...
        # Row 2 — header
        for col_idx, name in enumerate(self.COLUMNS, start=1):
        ...
        for doc in rows:
            ...
            values = [
                ...
                doc.get("doc_title", ""),
                doc.get("doc_code", ""),
                self._format_date(doc.get("doc_effective_date", "")),
                self._format_date(doc.get("doc_expiry_date", "")),
                self._format_date(doc.get("doc_issue_date", "")),
            ]
        ...
        # Footer — which part this file is, merged across all columns
        if num_parts > 1:
            row_idx += 1
            ws.merge_cells(start_row=row_idx, start_column=1, end_row=row_idx, end_column=len(self.COLUMNS))
            footer = ws.cell(row=row_idx, column=1, value=f"Phần {part}/{num_parts}")
            footer.font = Font(size=10, italic=True, bold=True)
            footer.alignment = Alignment(horizontal="center", wrap_text=True)

        buffer = BytesIO()
        wb.save(buffer)
        return buffer.getvalue(), stt

    def _scroll_rows(self, query, sort):
        """Yield each matching _source doc from the sorted scroll."""
        resp = es_client.search(
            index=ElasticConfig.ELASTIC_INDEX,
            query=query, sort=sort, scroll="2m", size=2000,
        )
    ...
    def _export_response(self, rows, total_count: int):
        """Build the download from an ordered row generator: a single .xlsx when
        the set fits in one part, otherwise a .zip of DSVB_<i>-<N>.xlsx parts with
        continuous STT. Shared by the text and semantic export endpoints."""
        import math
        from io import BytesIO
        import zipfile

        num_parts = max(1, math.ceil(total_count / self.PART_SIZE)) if total_count else 1

        if num_parts == 1:
            xlsx_bytes, _ = self._build_part(rows, 1, 1, 0)
            return send_file(
                BytesIO(xlsx_bytes),
                as_attachment=True,
                download_name="danh_sach_van_ban.xlsx",
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
            for part in range(1, num_parts + 1):
                xlsx_bytes, stt = self._build_part(rows, part, num_parts, stt)
                zf.writestr(f"DSVB_{part}-{num_parts}.xlsx", xlsx_bytes)
        zip_buffer.seek(0)
        return send_file(
            zip_buffer,
            as_attachment=True,
            download_name="danh_sach_van_ban.zip",
            mimetype="application/zip"
        )

    def post(self):
        bind_contextvars(task="DocumentTextSearchExportAPI")
        start_time = datetime.now()
        try:
            filters = request.get_json() or {}
            validated_filters = DocumentTextSearchAPI()._validate_filters(filters)

            # Always sort. Default to doc type A->Z then newest only when the
            # caller sent neither flag; otherwise respect exactly what was sent.
            if not validated_filters.get('sortByDocType') and not validated_filters.get('sortByIssuedDate'):
                validated_filters['sortByDocType'] = True
                validated_filters['sortByIssuedDate'] = True

            query = build_query(validated_filters)
            sort = DocumentTextSearchAPI._build_sort(validated_filters)
            total_count = es_client.count(index=ElasticConfig.ELASTIC_INDEX, body={"query": query})["count"]

            response = self._export_response(self._scroll_rows(query, sort), total_count)
            duration = (datetime.now() - start_time).total_seconds()
            logger.info("export_documents_success", action="post", **{"event.duration": duration, "event.status": "success"}, total=total_count)
            return response

        except ValueError as ve:
            duration = (datetime.now() - start_time).total_seconds()
            logger.error("export_documents_failed", action="post", **{"event.duration": duration, "event.status": "failed", "error.code": "400-VAL", "error.message": str(ve)}, exc_info=True)
            return {"code": "400", "message": f"Invalid input: {str(ve)}", "data": None, "error_code": "400-VAL", "status": False}, 400
        except Exception as e:
            duration = (datetime.now() - start_time).total_seconds()
            logger.error("export_documents_failed", action="post", **{"event.duration": duration, "event.status": "failed", "error.code": "500-SYS", "error.message": str(e)}, exc_info=True)
            return {"code": "500", "message": f"Internal server error: {str(e)}", "data": None, "error_code": "500-SYS", "status": False}, 500


class DocumentSemanticSearchExportAPI(DocumentTextSearchExportAPI):
    """Export semantic-search results to .xlsx — identical file format to the text
    export, only the row source differs.

    Rows come from the vector service (score-ordered IDs) instead of a sorted ES
    scroll. The set is bounded by top_k, so it is normally a single .xlsx.
    """

    def post(self):
        bind_contextvars(task="DocumentSemanticSearchExportAPI")
        start_time = datetime.now()
        try:
            filters = request.get_json() or {}
            semantic_api = DocumentSemanticSearchAPI()
            validated_filters = semantic_api._validate_filters(filters)

            # No query text → no vector ranking to apply. Degrade to the plain
            # filtered export (match_all + filters), exactly like the text export
            # and the interactive semantic search both do when text is blank.
            if not validated_filters.get('text'):
                return super().post()

            semantic_result = semantic_api._semantic_search(validated_filters)
            if not semantic_result:
                return {"code": "400", "message": "Semantic search failed", "data": None, "error_code": "400-VAL", "status": False}, 400

            # Vector hits (already score-ordered) -> codes + score map
            codes = [r['document_id'] for r in semantic_result['result']]
            score_map = {str(r['document_id']): r['score'] for r in semantic_result['result']}
            validated_filters['codes'] = codes

            # Fetch the matched docs from ES, then re-order by similarity score desc
            docs = []
            if codes:
                query = build_query_semantic_search(validated_filters)
                for batch in search(es_client, index=ElasticConfig.ELASTIC_INDEX,
                                    query=query, batch_size=1000, max_records=len(codes), skip=0):
                    for hit in batch:
                        hit = json.loads(hit) if isinstance(hit, str) else hit
                        docs.append(hit["_source"])
                docs.sort(key=lambda s: score_map.get(str(s.get("doc_id")), 0), reverse=True)

            response = self._export_response(iter(docs), len(docs))
            duration = (datetime.now() - start_time).total_seconds()
            logger.info("export_documents_success", action="post", **{"event.duration": duration, "event.status": "success"}, total=len(docs), semantic=True)
            return response
...
api.add_resource(DocumentTextSearchExportAPI, '/document/text-search/export')
api.add_resource(DocumentSemanticSearchExportAPI, '/document/semantic-search/export')
