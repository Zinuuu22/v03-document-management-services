...
KNOWLEDGE_NAME = f"NB_V03_Doc_Titles_{MODEL}_{VERSION}".format(MODEL=MODEL, VERSION=VERSION)
STATUS_FIELD = f"is_in_NB_V03_Doc_Titles_{MODEL}_{VERSION}".format(MODEL=MODEL, VERSION=VERSION)

MAX_WORKERS = int(MigrateConfig.MIGRATE_EMBEDDING_MAX_WORKERS)
BATCH_SIZE = 1000 

EMBEDDING_MODEL = EMBEDDING_MODELS[MODEL]
QDRANT = QdrantStorageManager(host='127.0.0.1', port=6333)
...
def embed_doc_title(
    ...
    """Embed and add document title to Qdrant vector database."""
    try:
        if not doc_title.strip():
            logger.debug(action="embed_doc_title", event="empty_title", document_id=document_id)
            return
        payloads = EMBEDDING_MODEL.embed_segments_batch(
            segments_id=[document_id],
            segments_index=[0],
        ...
        logger.error(action="embed_doc_title", event="title_embedding_failed", **{"error.code": "EXT", "error.message": str(e)}, document_id=document_id, exc_info=True)
        raise
...
def process_document(
    ...
    """Process a single document and add its title to Qdrant."""
    try:
        ...
        if not doc_title:
            logger.debug(action="process_document", event="no_title_found", document_id=document_id)
            return
...
def migrate_embedded_titles() -> None:
    """Migrate and embed document titles from law_documents to Qdrant vector database."""
    ...
    try:
        cursor: Cursor = law_documents_collection.find(
            ...
            {'doc_id': 1, 'doc_title': 1, 'embedding': 1, '_id': 1},
        ...
        if documents_batch:
            process_document_batch(documents_batch, processed_count_lock, error_count_lock)
        logger.info(action="migrate_embedded_titles", event="migration_completed", processed=processed_count, errors=error_count)
    except Exception as e:
        logger.error(action="migrate_embedded_titles", event="migration_failed", **{"error.code": "SYS", "error.message": str(e)}, exc_info=True)
        raise
    finally:
        cursor.close()
...
def main() -> None:
    """Run the document title embedding migration process."""
    logger.info(action="main", event="name_sentence_embedding_migration_started")
    start_time = time.time()
    try:
        migrate_embedded_titles()
        logger.info(action="main", event="name_sentence_embedding_migration_completed", duration=time.time() - start_time)
    except Exception as e:
        logger.error(action="main", event="name_sentence_embedding_migration_failed", **{"error.code": "SYS", "error.message": str(e)}, exc_info=True)
        raise
