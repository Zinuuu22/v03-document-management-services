...
# Constants
MODEL = MigrateConfig.MIGRATE_EMBEDDING_LONG_CONTEXT
VERSION = MigrateConfig.MIGRATE_EMBEDDING_VERSION
EMBEDDING_SIZE = MigrateConfig.MIGRATE_EMBEDDING_EMBEDDING_SIZE
KNOWLEDGE_NAME = f"NB_V03_Clauds_Level_Long_Context_{MODEL}_{VERSION}".format(MODEL=MODEL, VERSION=VERSION)
STATUS_FIELD = f"is_in_NB_V03_Clauds_Level_Long_Context_{MODEL}_{VERSION}".format(MODEL=MODEL, VERSION=VERSION)
...
def process_article(
    ...
    try:
        ...
        segments_index = []
        segments_id = []
        logger.debug(action="process_article", event="clause_count", article_id=article_id, count=len(clauds))    
        if len(clauds) == 0:
            ...
            for claud in clauds:
                logger.debug(action="process_article", event="clause_processing_started", index=claud['index'])
                claud_content = f"{article_title}\n{description.strip()}\n{claud['claud']}"
                segments_text.append(clean_text(claud_content))
                segments_index.append(claud['index'])
                segments_id.append(article_id)
