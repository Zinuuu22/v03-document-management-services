...
from core.v03.keywords_extractor.utils import get_prompts_extract_keywords, extract_keywords_wrapper, format_response, extract_keywords_wrapper_async
import httpx
import asyncio


def get_keywords(content, top_k=10):
    keywords = []
    llms_responses = []
    
    prompts = get_prompts_extract_keywords(content=content)            
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        futures = []
        for prompt in prompts:
            futures.append(executor.submit(extract_keywords_wrapper, prompt, llms_responses))
            concurrent.futures.wait(futures)
    output = format_response(llms_responses)
    if len(output) > top_k:
        keywords = output[:top_k]
    else:
        keywords = output     
    return keywords   

async def get_keywords_async(content, client: httpx.AsyncClient, semaphore: asyncio.Semaphore, top_k=10):
    keywords = []
    llms_responses = []
    
    prompts = get_prompts_extract_keywords(content=content)            
    tasks = [extract_keywords_wrapper_async(prompt, llms_responses, client, semaphore) for prompt in prompts]
...
PHỤ LỤC
MẪU BẢNG KÊ MUA BÁN, TRAO ĐỔI HÀNG HÓA QUA BIÊN GIỚI CỦA THƯƠNG NHÂN (Kèm theo Nghị định số 14/2018/NĐ-CP ngày 23 tháng 01 năm 2018 của Chính phủ)
TÊN THƯƠNG NHÂN -------
CỘNG HÒA XÃ HỘI CHỦ NGHĨA VIỆT NAM Độc lập - Tự do - Hạnh phúc  ---------------
…, ngày … tháng … năm …
...
Đại diện theo pháp luật của thương nhân (Ghi rõ chức danh, họ tên, ký tên và đóng dấu)"""
    ...
    logger.info("content_received", action="main", content_len=len(content))
    keywords = get_keywords(content)        
    logger.info("keywords_extracted", action="main", keywords=keywords)
